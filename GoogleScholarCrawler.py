...
class Article:
    ...
    def getInfo(self, article, driver):
        ...
        # 先找到 citation link
        cite_link = article.find_element(By.CSS_SELECTOR, "div.gs_fl a[href^='/scholar?cites']")
        # 提取最后的数字
        default['citation'] = cite_link.text.split()[-1]
        return default
...
def run(keywords, recursive=6):
    ...
    for i in keywords:
        ...
        for n in range(recursive):
            ...
            elements = driver.find_elements(By.CSS_SELECTOR, "div.gs_r.gs_or.gs_scl")  # find article boxes
            for e in elements:
                try:
            ...
            # 下一页
            try:
                next_button = driver.find_element(By.CSS_SELECTOR, "button.gs_btnPR")  # Google Scholar 的“下一页”按钮
                next_button.click()
                time.sleep(3)
            except:
                break   # 没有下一页则退出循环
